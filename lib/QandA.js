...
const choices = ["View All Department", "View All Roles",
    ...
    "Add An Employee", "Update An Employee Role", "Update Employee Managers", "ABORT | EXIT: I'm ALL DONE"];
...
function viewAllEmp(connection, init) {
    connection.query('SELECT employee.id, first_name, last_name, manager_id, roles.title, salary, department.name AS Dept_Name FROM employee JOIN roles ON roles.id = employee.roles_id JOIN department ON department.id = roles.department_id ', (err, res) => {
        err ? console.error(err) : console.log('\n')
        // console.log("use up / down to select another option")
        printTable(res);
...
//lol and here I thought I ran out of time yesterday. implementing bonus:
function updateEmpMngr(connection, init) {
    connection.query('SELECT employee.id, CONCAT(employee.first_name," ",employee.last_name) AS name, roles.title , employee.manager_id, CONCAT(manager.first_name," ",manager.last_name) AS Mngr_Name FROM employee AS employee LEFT JOIN employee AS manager ON manager.id = employee.manager_id LEFT JOIN roles ON roles.id = employee.roles_id', (err, res) => {
        printTable(res);
        console.log("^======Current Employees AND Managers=====^");
        connection.query('SELECT id, first_name AS Manager, last_name AS Name FROM employee WHERE manager_id is NULL', (err, respond) => {
            printTable(respond);
            console.log("^======Current Manager's ID======^")

            inquirer
                .prompt([
                    {
                        type: "list",
                        message: "Please choose from the following employeeID from current staff",
                        name: "employeeID",
                        choices: function () {
                            let empChoice = [];
                            // console.log(respond);
                            res.forEach(e => empChoice.push(e.id));
                            return empChoice;
                        }
                    },
                    {
                        ...
                        choices: function () {
                            let mangChoice = [];
                            mangChoice.push("promote to manager");
                            res.forEach(e => mangChoice.push(e.id));
                            return mangChoice;
                        }
                    },
                ]).then((answers) => {
                    if (answers.mngrID === "promote to manager") {
                        connection.query('UPDATE employee SET manager_id = NULL WHERE employee.id =?', [answers.employeeID], (err, res) => {
                            err ? console.error(err) : console.log(`Sucessfully updated EmployeeID: #${answers.employeeID}'s manager to #${answers.mngrID}`);
                            init();
                        });
                    } else {
                        connection.query('UPDATE employee SET manager_id =? WHERE employee.id =?', [answers.mngrID, answers.employeeID], (err, res) => {
                            err ? console.error(err) : console.log(`Sucessfully updated EmployeeID: #${answers.employeeID}'s manager to #${answers.mngrID}`);
                            init();
                        });
                    }
...
module.exports = {
    choices, viewAllDept, viewAllEmp, viewAllRole, addDept, addRole, addEmp, updateEmpRole, updateEmpMngr
}