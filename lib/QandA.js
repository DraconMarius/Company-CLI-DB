        ...
        inquirer
            .prompt([
                ...
                connection.query('SELECT id FROM roles WHERE title = ?', [Employee.roles_Name], (err, resRoleID) => {
                    ...
                    if (isNaN(Employee.manager_id) === false) {
...
function updateEmpRole(connection, init) {
    connection.query('SELECT first_name, employee.id AS EmployeeID, FROM employee LEFT JOIN roles ON roles.id = employee.roles_id', (err, respond) => {
        inquirer
            .prompt([
                {
                    type: "list",
                    message: "What is the new employee's First Nam?",
                    name: "fName",
                },
}
